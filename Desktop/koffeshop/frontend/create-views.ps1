# Script to create all remaining Vue view files for Koffeshop

$viewsPath = "c:\Users\user\Desktop\koffeshop\frontend\src\views"
...
Write-Host "Creating Vue view files..." -ForegroundColor Cyan

# Create files array with content
$files = @{
    "ProductsPage.vue" = @"
<template>
  <div class="products-page container py-5">
    ...
    <div class="filters-section mb-4 card-3d p-4 bg-white rounded">
      <div class="row g-3">
        ...
        <div class="col-md-4">
          <button @click="applyFilters" class="btn btn-primary btn-3d w-100">
    ...
    <div v-else class="row g-4">
      <div v-for="product in sortedProducts" :key="product.id" class="col-lg-3 col-md-4 col-sm-6">
        <div class="product-card card-3d">
          <div class="product-image-wrapper">
            <img :src="product.image || '/placeholder.jpg'" :alt="product.name" class="product-image" />
            <button @click="toggleWishlist(product)" class="wishlist-btn btn-3d" :class="{ 'active': wishlistStore.isInWishlist(product.id) }">
          ...
          <router-link :to="`/products/`+product.id" class="product-overlay">
    ...
    <div v-else-if="product" class="row g-5">
      <div class="col-lg-6">
        <div class="product-image-container card-3d">
          <img :src="product.image || '/placeholder.jpg'" :alt="product.name" class="img-fluid rounded-3 shadow-3d" />
        </div>
      </div>
...
foreach ($fileName in $files.Keys) {
    $filePath = Join-Path $viewsPath $fileName
    Write-Host "Creating $fileName..." -ForegroundColor Yellow
    $files[$fileName] | Out-File -FilePath $filePath -Encoding UTF8 -Force
    Write-Host "  ✓ Created $fileName" -ForegroundColor Green
}

Write-Host "`nAll view files created successfully!" -ForegroundColor Green
"@